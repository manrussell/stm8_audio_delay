 ...
 144                     ; 104 void main( void )
 144                     ; 105 {
 146                     	switch	.text
 147  0000               _main:
 149  0000 520f          	subw	sp,#15
 150       0000000f      OFST:	set	15
 153                     ; 106     uint16_t adc_leftChannel = 0;
 155                     ; 107     uint16_t adc_feedback    = 0;
 157                     ; 108     uint16_t adc_delay       = 0;
 159                     ; 109     uint8_t  mapd_value      = 0;    // mapped adc valu
 161                     ; 110     uint8_t  read_val        = 0;    // read val from ram
 163  0002 0f08          	clr	(OFST-7,sp)
 165                     ; 111     uint16_t write_addr      = 0;    // write addr in ram
 167  0004 5f            	clrw	x
 168  0005 1f0c          	ldw	(OFST-3,sp),x
 170                     ; 112     uint16_t read_addr       = 0;    // read addr in val
 172  0007 1f09          	ldw	(OFST-6,sp),x
 174                     ; 113     uint16_t delay           = 110;  // length of delay in samples
 176                     ; 114     uint8_t  res             = 0;
 178                     ; 116     clock_setup();
 ...
 290  0070 85            	popw	x
 291                     ; 196         MCP4901_DAC_write(read_val);
 ...
 373                     ; 217 uint8_t TEST_ram_test_001(void)
 373                     ; 218 {
 374                     	switch	.text
 375  0083               _TEST_ram_test_001:
 377  0083 5205          	subw	sp,#5
 378       00000005      OFST:	set	5
 381                     ; 219     uint8_t cnt = 0;
 383                     ; 220     uint8_t data = 0;
 386                     ; 221     uint8_t wdata = 0;
 388                     ; 222     uint16_t addr = 0;
 390                     ; 225     data = BYTE_MODE;
 392  0085 0f03          	clr	(OFST-2,sp)
 394                     ; 226     MCP_23K256_write_status_register(data);
 396  0087 4f            	clr	a
 397  0088 cd03ab        	call	_MCP_23K256_write_status_register
 399                     ; 229     MCP_23K256_read_status_register(&data);
 ...
 415                     ; 276     MCP_23K256_RAM_write_byte(addr, wdata);
 417  0099 4b02          	push	#2
 418  009b cd03e8        	call	_MCP_23K256_RAM_write_byte
 420  009e 84            	pop	a
 421                     ; 279     MCP_23K256_RAM_read_byte(addr, &data);
 ...
 483  00dd 85            	popw	x
 484                     ; 307     if (wdata != data)
 486                     ; 313     for(addr = 0; addr<3; addr++)
 488  00de 5f            	clrw	x
 489  00df 1f04          	ldw	(OFST-1,sp),x
 491  00e1               L141:
 492                     ; 315         MCP_23K256_RAM_read_byte(addr, &data);
 ...
 500  00eb 85            	popw	x
 501                     ; 313     for(addr = 0; addr<3; addr++)
 ...
 620  011c               L302:
 621                     ; 339         for(addr = 0; addr<255; addr++)
 623  011c 5f            	clrw	x
 624  011d 1f03          	ldw	(OFST-1,sp),x
 626  011f               L702:
 627                     ; 341             MCP_23K256_RAM_read_byte(addr, &data);
 ...
 819                     ; 402         MCP_23K256_RAM_write_byte(addr, mapd_value);
 821  0170 88            	push	a
 822  0171 1e04          	ldw	x,(OFST-1,sp)
 823  0173 cd03e8        	call	_MCP_23K256_RAM_write_byte
 825  0176 84            	pop	a
 826                     ; 405         mapd_value = 0; // check value
 828  0177 0f05          	clr	(OFST+0,sp)
 830                     ; 406         MCP_23K256_RAM_read_byte(addr, &mapd_value);
 ...
 845                     ; 411         MCP4901_DAC_write(mapd_value);
 ...
 953                     ; 421 void TEST_adc_to_ram_to_dac_with_delay(void)
 953                     ; 422 {
 954                     	switch	.text
 955  0196               _TEST_adc_to_ram_to_dac_with_delay:
 957  0196 520b          	subw	sp,#11
 958       0000000b      OFST:	set	11
 961                     ; 423     uint16_t adc_val = 0;
 963                     ; 424     uint8_t mapd_value = 0; // mapped adc valu
 965                     ; 425     uint8_t read_val = 0;   //read val from ram
 967  0198 0f05          	clr	(OFST-6,sp)
 969                     ; 426     uint16_t write_addr = 0;    //write addr in ram
 971  019a 5f            	clrw	x
 972  019b 1f0a          	ldw	(OFST-1,sp),x
 974                     ; 427     uint16_t read_addr = 0; // read addr in val
 976  019d 1f06          	ldw	(OFST-5,sp),x
 978                     ; 428     uint16_t delay = 110; // length of delay in samples
 980  019f ae006e        	ldw	x,#110
 981  01a2 1f08          	ldw	(OFST-3,sp),x
 983                     ; 429     uint8_t res = 0;
 985  01a4               L153:
 986                     ; 435         ADC1_StartConversion();
 988  01a4 cd0000        	call	_ADC1_StartConversion
 991  01a7               L753:
 992                     ; 436         while(ADC1_GetFlagStatus(ADC1_FLAG_EOC) == FALSE);
 994  01a7 a680          	ld	a,#128
 995  01a9 cd0000        	call	_ADC1_GetFlagStatus
 997  01ac 4d            	tnz	a
 998  01ad 27f8          	jreq	L753
 999                     ; 438         adc_val = ADC1_GetConversionValue(); // for led knob
1001  01af cd0000        	call	_ADC1_GetConversionValue
1003  01b2 1f02          	ldw	(OFST-9,sp),x
1005                     ; 439         ADC1_ClearFlag(ADC1_FLAG_EOC);
1007  01b4 a680          	ld	a,#128
1008  01b6 cd0000        	call	_ADC1_ClearFlag
1010                     ; 446         mapd_value = 0.25 *(float)adc_val ; // do i need the float or not ???
...
1044  01e3 85            	popw	x
1045                     ; 467         MCP4901_DAC_write(read_val);
...
1291  0258 85            	popw	x
1292                     ; 551         MCP4901_DAC_write(read_val);
...
1498                     ; 609 }
1501  02ed 81            	ret	
1529                     ; 637 void ADC1_setup(void)
1529                     ; 638 {
1530                     	switch	.text
1531  02ee               _ADC1_setup:
1535                     ; 639   ADC1_DeInit();
1537  02ee cd0000        	call	_ADC1_DeInit
1539                     ; 641   ADC1_Init(ADC1_CONVERSIONMODE_SINGLE, \
1539                     ; 642   ADC1_CHANNEL_0,\
1539                     ; 643   ADC1_PRESSEL_FCPU_D18, \
1539                     ; 644   ADC1_EXTTRIG_GPIO, \
1539                     ; 645   DISABLE, \
1539                     ; 646   ADC1_ALIGN_RIGHT, \
1539                     ; 647   ADC1_SCHMITTTRIG_CHANNEL0, \
1539                     ; 648   DISABLE);
...
1551                     ; 650   ADC1_Init(ADC1_CONVERSIONMODE_SINGLE, \
1551                     ; 651   ADC1_CHANNEL_1,\
1551                     ; 652   ADC1_PRESSEL_FCPU_D18, \
1551                     ; 653   ADC1_EXTTRIG_GPIO, \
1551                     ; 654   DISABLE, \
1551                     ; 655   ADC1_ALIGN_RIGHT, \
1551                     ; 656   ADC1_SCHMITTTRIG_CHANNEL1, \
1551                     ; 657   DISABLE);
...
1563                     ; 659   ADC1_ConversionConfig(ADC1_CONVERSIONMODE_SINGLE, ((ADC1_Channel_TypeDef)(ADC1_CHANNEL_0 | ADC1_CHANNEL_1)), ADC1_ALIGN_RIGHT);
1565  0317 4b08          	push	#8
1566  0319 ae0001        	ldw	x,#1
1567  031c cd0000        	call	_ADC1_ConversionConfig
1569  031f 84            	pop	a
1570                     ; 661   ADC1_DataBufferCmd(ENABLE);
1572  0320 a601          	ld	a,#1
1573  0322 cd0000        	call	_ADC1_DataBufferCmd
1575                     ; 662   ADC1_Cmd(ENABLE);
1577  0325 a601          	ld	a,#1
1579                     ; 664 }
1582  0327 cc0000        	jp	_ADC1_Cmd
1605                     ; 669 void TIM2_setup(void)
1605                     ; 670 {
1606                     	switch	.text
1607  032a               _TIM2_setup:
1611                     ; 682 }
1614  032a 81            	ret	
1640                     ; 687 void SPI_setup(void)
1640                     ; 688 {
1641                     	switch	.text
1642  032b               _SPI_setup:
1646                     ; 689   SPI_DeInit();
1648  032b cd0000        	call	_SPI_DeInit
1650                     ; 694   SPI_Init(SPI_FIRSTBIT_MSB, \
1650                     ; 695     SPI_BAUDRATEPRESCALER_16, \
1650                     ; 696     SPI_MODE_MASTER, \
1650                     ; 697     SPI_CLOCKPOLARITY_LOW, \
1650                     ; 698     SPI_CLOCKPHASE_1EDGE, \
1650                     ; 699     SPI_DATADIRECTION_2LINES_FULLDUPLEX, \
1650                     ; 700     SPI_NSS_SOFT, \
1650                     ; 701     0x0);
...
1837                     ; 739   GPIO_WriteHigh(RAM_CS_port, RAM_CS_pin);
...
1847  03aa 81            	ret	
1887                     ; 749 void MCP_23K256_write_status_register(uint8_t data)
1887                     ; 750 {
1888                     	switch	.text
...
1943                     ; 768   GPIO_WriteHigh(RAM_CS_port, RAM_CS_pin);
...
1953  03e7 81            	ret	
2011                     ; 779 void MCP_23K256_RAM_write_byte(uint16_t address, unsigned char value)
2011                     ; 780 {
2012                     	switch	.text
...
2107                     ; 809   GPIO_WriteHigh(RAM_CS_port, RAM_CS_pin);
...
2118  0444 81            	ret	
2178                     ; 819 void MCP_23K256_RAM_read_byte(uint16_t address, unsigned char *value)
2178                     ; 820 {
2179                     	switch	.text
...
2298                     ; 858   GPIO_WriteHigh(RAM_CS_port, RAM_CS_pin);
...
2480                     ; 930   GPIO_WriteHigh(DAC_CS_port, DAC_CS_pin);
