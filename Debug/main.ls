 ...
 307                     ; 116                 break;
 309                     ; 117             case 7:
 309                     ; 118                 /* do average of ADC pots values */
 309                     ; 119                 // how long does this take?
 309                     ; 120                 // this seems to break, is the maths? do i need a bigger type, or to cast?
 309                     ; 121                 //delay = ( delay_length_samples[0] + delay_length_samples[1] + delay_length_samples[2] + delay_length_samples[3] ) / NUM_Of_ADCPOT_SAMPLES;
 309                     ; 122                 break;
 311  007e               L31:
 312                     ; 123             case 8:
 312                     ; 124                 /* Sample Audio ADC, process, write to DAC */
 312                     ; 125                 //sample audio input
 312                     ; 126                 ADC1_ScanModeCmd( ENABLE );
 314  007e 4c            	inc	a
 315  007f cd0000        	call	_ADC1_ScanModeCmd
 317                     ; 127                 ADC1_StartConversion( );
 319  0082 cd0000        	call	_ADC1_StartConversion
 322  0085               L131:
 323                     ; 128                 while( ADC1_GetFlagStatus( ADC1_FLAG_EOC ) == FALSE );
 325  0085 a680          	ld	a,#128
 326  0087 cd0000        	call	_ADC1_GetFlagStatus
 328  008a 4d            	tnz	a
 329  008b 27f8          	jreq	L131
 330                     ; 129                 ADC1_ClearFlag(ADC1_FLAG_EOC);
 332  008d a680          	ld	a,#128
 333  008f cd0000        	call	_ADC1_ClearFlag
 335                     ; 130                 adc_leftChannel = ADC1_GetBufferValue( 0 ); //ADC_LEFTCHANNEL );
 337  0092 4f            	clr	a
 338  0093 cd0000        	call	_ADC1_GetBufferValue
 340  0096 1f14          	ldw	(OFST-3,sp),x
 342                     ; 137                 mapd_value = (adc_leftChannel >> 2);
 344  0098 54            	srlw	x
 345  0099 54            	srlw	x
 346  009a 01            	rrwa	x,a
 347  009b 6b11          	ld	(OFST-6,sp),a
 349                     ; 140                 mapd_value = mapd_value/2 +  read_val/2;
 351  009d 5f            	clrw	x
 ...
 386  00c7 85            	popw	x
 387                     ; 161                     MCP4901_DAC_write(read_val);
 ...
 687                     ; 263   ADC1_Init(ADC1_CONVERSIONMODE_SINGLE, \
 687                     ; 264   ADC1_CHANNEL_0,\
 687                     ; 265   ADC1_PRESSEL_FCPU_D18, \
 687                     ; 266   ADC1_EXTTRIG_GPIO, \
 687                     ; 267   DISABLE, \
 687                     ; 268   ADC1_ALIGN_RIGHT, \
 687                     ; 269   ADC1_SCHMITTTRIG_CHANNEL0, \
 687                     ; 270   DISABLE);
 ...
 699                     ; 272   ADC1_Init(ADC1_CONVERSIONMODE_SINGLE, \
 699                     ; 273   ADC1_CHANNEL_1,\
 699                     ; 274   ADC1_PRESSEL_FCPU_D18, \
 699                     ; 275   ADC1_EXTTRIG_GPIO, \
 699                     ; 276   DISABLE, \
 699                     ; 277   ADC1_ALIGN_RIGHT, \
 699                     ; 278   ADC1_SCHMITTTRIG_CHANNEL1, \
 699                     ; 279   DISABLE);
 ...
 711                     ; 281   ADC1_ConversionConfig(ADC1_CONVERSIONMODE_SINGLE, ((ADC1_Channel_TypeDef)(ADC1_CHANNEL_0 | ADC1_CHANNEL_1)), ADC1_ALIGN_RIGHT);
 713  0189 4b08          	push	#8
 714  018b ae0001        	ldw	x,#1
 715  018e cd0000        	call	_ADC1_ConversionConfig
 717  0191 84            	pop	a
 718                     ; 283   ADC1_DataBufferCmd(ENABLE);
 720  0192 a601          	ld	a,#1
 721  0194 cd0000        	call	_ADC1_DataBufferCmd
 723                     ; 284   ADC1_Cmd(ENABLE);
 725  0197 a601          	ld	a,#1
 727                     ; 286 }
 730  0199 cc0000        	jp	_ADC1_Cmd
 757                     ; 296 void SPI_setup(void)
 757                     ; 297 {
 758                     	switch	.text
 759  019c               _SPI_setup:
 763                     ; 298   GPIO_Init(SPI_PORT, (GPIO_Pin_TypeDef)(SPI_CLK | SPI_MOSI | SPI_MISO), GPIO_MODE_OUT_PP_HIGH_FAST);
 ...
 775                     ; 302   SPI_Init(SPI_FIRSTBIT_MSB, \
 775                     ; 303     SPI_BAUDRATEPRESCALER_16, \
 775                     ; 304     SPI_MODE_MASTER, \
 775                     ; 305     SPI_CLOCKPOLARITY_LOW, \
 775                     ; 306     SPI_CLOCKPHASE_1EDGE, \
 775                     ; 307     SPI_DATADIRECTION_2LINES_FULLDUPLEX, \
 775                     ; 308     SPI_NSS_SOFT, \
 775                     ; 309     0x0);
